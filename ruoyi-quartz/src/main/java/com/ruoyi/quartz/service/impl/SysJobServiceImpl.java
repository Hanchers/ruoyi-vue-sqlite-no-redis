...
public class SysJobServiceImpl extends ServiceImpl<SysJobMapper, SysJob> implements ISysJobService {
    ...
    public void init() throws SchedulerException, TaskException {
        scheduler.clear();
        List<SysJob> jobList = mapper.selectAll();
    ...
    public List<SysJob> selectJobList(SysJob job) {
        return mapper.selectJobList(job);
    }
    ...
    public int insertJob(SysJob job) throws SchedulerException, TaskException {
        // SQLite 不支持复合主键列的自增，手动生成 job_id
        if (job.getJobId() == null || job.getJobId() == 0) {
            Long maxJobId = queryChain().select("COALESCE(MAX(job_id), 0) + 1").objAs(Long.class);
